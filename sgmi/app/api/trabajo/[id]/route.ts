...
export async function GET(
  ...
  try {
    ...
    const res = await TrabajoController.getById(parseInt(id));
...
export async function PUT(
  ...
  try {
    ...
    const res = await TrabajoController.update(parseInt(id), body);
    return NextResponse.json(res, {
      status: res.success ? 200 : res.error === "No autorizado" ? 403 : 400,
    });
  } catch (e: any) {
    return NextResponse.json(
      { success: false, error: "Error interno" },
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params;
    const res = await TrabajoController.delete(parseInt(id));
    return NextResponse.json(res, {
      status: res.success ? 200 : res.error === "No autorizado" ? 403 : 404,
    });
  } catch (e: any) {
    return NextResponse.json(
      { success: false, error: "Error interno" },
